...
public class Boat {
	...
	/*
	 * selfTest() tests our Task VI. Uncomment BoatGrader b = new BoatGrader() and
	 * one or more tests to see how boat behaves respectively.
	 */
...
//	begin(23, 17, b);
	...
	/*
	 * begin() contains the Task VI code. It takes in the starting amount of adults
	 * on Oahu, the starting amount of children on Oahu, and a BoatGrader object
	 * that we assign to our static BoatGrader object so we can store our
	 * itinerary/solution.
	 */
	...
	/*
	 * AdultItinerary() plans how adult threads will get to Molokai. They will be
	 * spawned by begin() and will immediately sleep on Oahu. Once all children
	 * threads have finished the trip to Molokai, two threads will remain to ferry
	 * the boat back and forth so adults can make the trip to Molokai. Once all
	 * adults are on Molokai, the final two children threads will call return and
	 * change the while loop bool not_done to false.
	 */
	static void AdultItinerary() {
		...
		while (not_done) {
			 ...
			 * specifications, this adult can know how many children are on Molokai because
			 * it has seen them leave Oahu. It can also see children on Oahu. If the boat is
			 * not on Oaku or if there is more than one child on Oahu...
			 */
			while (!boat_is_on_oahu || (total_children - num_child_on_molokai) > 1 || (children_on_boat != 0)) {
	...
	/*
	 * ChildItinerary() plans how child threads will get to Molokai. They will be
	 * spawned by begin() and will immediately move to Oahu. Once all children
	 * threads have finished the trip to Molokai, two threads will remain to ferry
	 * the boat back and forth so adults can make the trip to Molokai. Once all
	 * adults are on Molokai, the final two children threads will call return and
	 * change the while loop bool not_done to false.
