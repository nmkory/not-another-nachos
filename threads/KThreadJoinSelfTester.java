...
public class KThreadJoinSelfTester {
	...
	public static void selfTest4() {
		...
		for (int i = 0; i < 5; i++) {
			new KThread(joinerRun).setName("Joiner " + Integer.toString(i + 1)).fork();
		}
	...
	public static void selfTest5() {
		...
		Lib.debug(dbgThread, KThread.currentThread().getName() + " has spawned " + joining_thread.getName()
				+ " and will join on it");
		joining_thread.join();

		Lib.debug(dbgThread, KThread.currentThread().getName() + " has resumed control");
	...
	static void joinSelf() {
		Lib.debug(dbgThread, "The " + KThread.currentThread().getName() + " is going to try to join on itself");
		try {
			KThread.currentThread().join();
		} catch (Throwable e) {
			Lib.debug(dbgThread, KThread.currentThread().getName() + " correctly failed to join itself");
		}

	} // joinSelf()

	/**
	 * selfTest2: Function that runs in runnable obj to wait on ping test. Thread
	 * will create a ping test thread using pingTestRun and then join on it. Then
	 * try to join on it again.
	 */
	static void finishAndJoin() {
		Lib.debug(dbgThread, "The " + KThread.currentThread().getName() + " is about to spawn a ping_test thread");

		KThread ping_test = new KThread(pingTestRun);
		ping_test.setName("ping_test");
		ping_test.fork();

		Lib.debug(dbgThread, "The " + KThread.currentThread().getName() + " has spawned " + ping_test.getName()
				+ " and will join on it");
	...
	/**
	 * selfTest3: Function that runs in runnable obj to wait on ping test. Thread
	 * will create a ping test thread using pingTestRun and then join on it.
	 */
	static void spawnAndJoin() {
		...
		Lib.debug(dbgThread, "The " + KThread.currentThread().getName() + " has resumed control ");
	} // spawnAndJoin()

	/**
	 * selfTest2 & 3: Function that runs in runnable obj. Runs a loop that announces
	 * index and yields. Other threads will wait for this to finish when they join
	 * on it.
	 */
	static void pingTestRunner() {
		Lib.debug(dbgThread, "Thread " + KThread.currentThread().getName() + " is about to run pingTest");

		for (int i = 0; i < 5; i++) { // You may adjust the number of iterations
			Lib.debug(dbgThread, "Thread " + KThread.currentThread().getName() + " looped " + (i + 1) + " times");
			KThread.yield();
		}

		Lib.debug(dbgThread, "Thread " + KThread.currentThread().getName() + " has ended pingTest");
	} // pingTestRunner()
	...
	static void joinerRunner() {
		Lib.debug(dbgThread, "The " + KThread.currentThread().getName() + " is going to join on join_on_me");

		join_on_me.join();

		Lib.debug(dbgThread, "The " + KThread.currentThread().getName() + " has finished");
