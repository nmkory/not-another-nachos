...
public class PrioSchedulerSelfTester {
	...
	/**
	 * A low priority thread, a high priority thread, and a mid priority thread do
	 * some stuff. The threads should run in this order: High, Mid, Low
	 */
	...
	/**
	 * A low priority thread and two threads of equal priority do some stuff. The
	 * two threads of equal priority should run in the order they were forked. When
	 * the two threads of equal priority finish running, the low priority thread
	 * should run.
	 */
	...
	/**
	 * Threads with a mix of priorities run simultaneously. One thread has high
	 * priority, two threads have mid (but equal) priority, and one thread has low
	 * priority. The threads should run from highest to lowest priority with threads
	 * of equal priority context switching back and forth.
	 */
	...
	/**
	 * Same as selfTest3() except instead of putting the threads into the
	 * readyQueue, we make the threads join theThreadThatDoesStuff When
	 * theThreadThatDoesStuff finishes running, the threads should run in the same
	 * order as in selfTest3()
	 */
	...
	/**
	 * A thread with a high priority has their priority decreased while it's waiting
	 * for theThreadThatDoesStuff The priority is decreased below the priority of
	 * all other threads, so the other threads should run first. The result should
	 * be that the threads are originally scheduled in the order A, C, B but then
	 * are scheduled in the order C, B, A
	 */
	public static void selfTest5() {
		...
		changePriorityOfThread(threadA, 2); // Change the priority of Thread A to 2 making it the thread with the least
											// priority
	}

	/**
	 * A trio of threads are running with equal priority. While running, one of the
	 * threads lowers its own priority while another increases its own priority. The
	 * threads should finish in this order: 1) The thread that increased its
	 * priority 2) The thread that didn't change priority 3) The thread that
	 * decreased its priority.
	 */
	...
	/**
	 * A thread with Priority 4 and a thread with Priority 6 attempt to acquire a
	 * resource held by theThreadThatDoesStuff the effectivePriority of
	 * theThreadThatDoesStuff should first increase to 4 then increase to 6.
	 */
	public static void selfTest7() {
		...
		changePriorityOfThread(KThread.currentThread(), 4); // Increase the priority of the main thread to 4 so that
															// main can create the Priority 6 thread
	...
	/**
	 * A thread with Priority 1 acquires a resource and is put into the ready queue
	 * behind a thread with Priority 4 A thread with Priority 6 then attempts to
	 * acquire the resource held by the Priority 2 thread resulting in a case of
	 * Priority Inversion The Priority 1 thread should return an effectivePriority
	 * of 6 and be the next thread to run
	 */
	public static void selfTest8() {
		...
		changePriorityOfThread(threadC, 4); // A temporary priority so that Thread C runs after one iteration from
											// Thread B
		threadC.fork();

		/*
		 * At this point, Thread C will run and attempt to acquire the resource. Since
		 * that resource is currently being held by Thread A, Thread C will wait for
		 * access resulting in a case of Priority Inversion The next thread to run
		 * should be Thread A with an effective priority of 6 rather than Thread B with
		 * a priority of 4.
		 */
	}
	...
	/**
	 * A function that makes a thread acquire a resource then do some stuff. The
	 * thread will set its priority and print its effective priority each time it
	 * does stuff
	 */
	static void doStuffWithResource(int numIterations, int priority) {
		 ...
		 * 
		 * if (someResource.owner != null) { Lib.debug(dbgThread,
		 * "Resource busy! Must wait for access...");
		 * someResource.waitForAccess(KThread.currentThread()); KThread.sleep(); } else
		 ...
		 * 
		 * someResource.owner = someResource.pickNextThread(); if (someResource.owner !=
		 * null) { someResource.nextThread().ready(); }
		 * 
		 * Machine.interrupt().restore(intStatus);
		 */
	...
	/**
	 * A function that makes a thread increase its priority while it's running This
	 * increase in priority occurs early in the ping test
	 */
	...
	/**
	 * A function that makes a thread increase its priority while it's running This
	 * decrease in priority occurs late in the ping test
	 */
	...
	/**
	 * A function that makes a thread join theThreadThatDoesStuff before doing some
	 * stuff themselves
	 */
	static void joinThenDoStuff() {
		...
		Lib.debug(dbgThread,
				"I am " + KThread.currentThread().getName() + " and I have resumed control. Time to do some stuff!");
		doStuff(10);
	}

	/**
	 * A function that makes a thread set its priority first before joining
	...
	static void setPriorityThenJoin(int priority) {
		...
		Lib.debug(dbgThread, "Done updating my priority. My effective priority is now "
				+ ThreadedKernel.scheduler.getEffectivePriority(KThread.currentThread()));
...
//	
	...
	/**
	 * A Runnable for a thread that will first join on theThreadThatDoesStuff before
	 * doing stuff itself. These threads have default priority
	 */
	private static Runnable joinThenDoStuff = new Runnable() {
		public void run() {
			Lib.debug(dbgThread,
					"I am " + KThread.currentThread().getName() + " and I'm going to join before doing stuff!");
			joinThenDoStuff();
		}
	};

	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have the minimum priority (0)
	 */
	private static Runnable joinWithMinPriority = new Runnable() {
		public void run() {
			Lib.debug(dbgThread,
					"I am " + KThread.currentThread().getName() + " and I'm going to join with the minimum priority!");
	...
	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have a priority of 1 This runnable is
	 * technically no different than joinThenDoStuff
	 */
	private static Runnable joinWithPriority1 = new Runnable() {
		public void run() {
			int priority = 1;
			Lib.debug(dbgThread, "I am " + KThread.currentThread().getName()
					+ " and I'm going to join with a priority of " + priority);
	...
	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have a priority of 2
	 */
	...
	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have a priority of 3
	 */
	...
	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have a priority of 4
	 */
	...
	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have a priority of 5
	 */
	...
	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have a priority of 6
	 */
	...
	/**
	 * A Runnable for a thread that will set its priority before joining
	 * theThreadThatDoesStuff These threads have the maximum priority (7)
	 */
	private static Runnable joinWithMaxPriority = new Runnable() {
		public void run() {
			Lib.debug(dbgThread,
					"I am " + KThread.currentThread().getName() + " and I'm going to join with the maximum priority!");
	...
	/**
	 * A Runnable for a thread to grab a resource and do some stuff These threads
	 * will also print their effective priority while running
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have the minimum priority
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have a priority of 1
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have a priority of 2
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have a priority of 3
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have a priority of 4
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have a priority of 5
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have a priority of 6
	 */
	...
	/**
	 * A Runnable for a thread to do some stuff immediately while printing its
	 * effective priority. These threads have the maximum priority (7)
	 */
	...
	// private static PriorityQueue someResource = (PriorityQueue)
